...
main(int argc, char* argv[])
{
    ...
    UdpEchoServerHelper sidelinkSink(port);
     ...
     */
    std::string exampleName = simTag + "-" + "eris";
    // Datebase setup
    ...
    if (!useIPv6)
    {
        ...
        for (uint32_t ac = 0; ac < clientApps.GetN(); ac++)
        {
            ...
            clientApps.Get(ac)->TraceConnect("TxWithAddresses",
                                             "tx",
        ...
        for (uint32_t ac = 0; ac < serverApps.GetN(); ac++)
        {
            ...
            serverApps.Get(ac)->TraceConnect("RxWithAddresses",
                                             "rx",
                                             MakeBoundCallback(&UePacketTraceDb,
                                                               &pktStats,
